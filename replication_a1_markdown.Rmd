...
rolling_forecast_arma <- function(data, series_name, order_ar=1, order_ma=1, initial_end=as.Date('1985-12-01'), final_date=as.Date('2002-12-01'), window_size=4) {
  ...
  for (y in year(initial_end):year(final_date)) {
    ...
    test_data <- data %>% filter(FirstDate > as.Date(str_c(y, '-12-01')))  # Use the next quarter for testing
...
rolling_forecast_ar_p <- function(data, series_name, initial_end=as.Date('1985-12-01'), final_date=as.Date('2002-12-01'), window_size=4) {
  # adding a forecast vector to store results
  forecasts <- numeric((year(final_date) - year(initial_end))*window_size)
...
for (series in inflation_series) {
  arma11_forecasts[[series]] <- rolling_forecast_arma(df_inflation_authors, series)
}
