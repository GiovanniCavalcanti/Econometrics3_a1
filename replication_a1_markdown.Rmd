...
# y <- data %>% filter(FirstDate <= as.Date(str_c(1987, '-12-01'))) %>% select(inflation_punew)
# model <- arima(y, order = c(3, 0, 0))
# a <- as.numeric(forecast(model, h=4)$mean)
# a


rolling_forecast_ar_p(df_inflation_authors, inflation_series[1], return_rmse = T)
